...
function buildChatSystemPrompt(context) {
  ...
  return [
    ...
    '- When the lifter asks what to train (or names a preference like "upper body" or "legs"), give the full session in the FIRST reply — commit to the movements, sets, reps, and RIR. Format: one short sentence of context, then each exercise on its own line: "Exercise — {load} × Y reps × Z sets @ RIR N", where {load} is a specific weight ONLY when the snapshot supplies one for that lift (see the PRESCRIPTION LOADS rule below); otherwise write the line as "Exercise — Y reps × Z sets @ RIR N" with no weight. Only ask a question if there is a genuine reason not to proceed (injury conflict, missing info you cannot infer).',
