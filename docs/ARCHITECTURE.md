...
**Superseded in part, 2026-08-07.** The owner authorized one bounded migration: the workout hot path moves to Supabase, and Google Sheets becomes an export mirror for those concepts only. The instruction is recorded in [`docs/ATLAS_V1_EXECUTION_PLAN.md`](./ATLAS_V1_EXECUTION_PLAN.md); the design is [`docs/SUPABASE_HOT_PATH_MIGRATION.md`](./SUPABASE_HOT_PATH_MIGRATION.md); the per-concept authority is [`docs/ATLAS_SYSTEM_AUTHORITY.md`](./ATLAS_SYSTEM_AUTHORITY.md) concept 18. Sheets remains the sole live authority until **PR S4**, which moves reads and writes together (owner ruling D5); PR S3 moves neither. Everything outside those seven concepts stays Sheets-primary.
