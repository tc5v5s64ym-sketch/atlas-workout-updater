...
- **`public/` is gitignored build output (PR-22)** — `src/app/` is the only committed frontend tree. `public/` is regenerated by `npm run build`, guaranteed to run before the server ever starts serving it via a `prestart` npm lifecycle hook (`package.json`): `npm start` always runs `prestart` (`npm run build`) first, so this holds regardless of what a platform's separately-configured "Build Command" does — the guarantee lives in the repo (`Procfile`: `web: npm start`), not in an external dashboard setting. `vite` is a regular `dependencies` entry (not `devDependencies`) so it installs in a production-only (`--omit=dev`) install too. CI builds fresh and asserts the expected output files exist rather than diffing against a committed copy (there is none). `public/build-info.json` (git-untracked, like the rest of `public/`) is written immediately after the Vite copy by the same `build` script and is preserved across rebuilds (`emptyOutDir: false`).
