...
- **`test_mode`** (request field): absent = live write (invariant W2); `"true"` = dry-run (`sheet_written:false, no_write_confirmed:true`); ambiguous values fail closed. Enforced in `index.js` at every write route. (`ATLAS_TEST_MODE` is a phantom token — no code references it; ignore any doc that cites it.)
