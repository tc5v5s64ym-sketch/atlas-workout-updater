...
- **Initiates live app testing** — app tests are owner-initiated, not an automatic stop. (**Owner amendment 2026-07-10**, `docs/OWNER_CHECKIN_RULES.md`: the builder is additionally authorized to perform its own live verification against production — read-only/dry-run by default per `docs/MISSION_CONTROL.md`, a test-marked same-session-reverted write only as a last resort; gym-session tests remain owner-only.) The builder flags `owner-live-test` and supplies a live test script, but keeps going; the owner calls the hold and says stop when they want to test. The loop runs until the owner says stop or an owner-decision criterion (2–8) is hit. **Live tests require deployed code and are post-merge validations — they are never pre-merge gates.** The `owner-live-test` flag does NOT set `Owner action required: Yes` on the merge card and does NOT produce `hold-for-owner`; the PR merges normally.
