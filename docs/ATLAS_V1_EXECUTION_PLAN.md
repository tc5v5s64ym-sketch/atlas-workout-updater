...
### ATLAS CONTRACT / SYSTEMS REVIEW 2026-08-08 (seventeenth required) — exact head `ad189075bd54377fac73c1af8704b43403a5369c`: BLOCKING

The first required review of an **implementation** head rather than a paper one. CI was green on that head; the review found **two P1 contract/authority defects the proofs did not cover**. Both are corrected. The architecture disposition is unchanged — authority-defect migration, Sheets plus `services/idempotency.js` the current winner, Supabase the intended one, `S4` the named removal point — and the direction is confirmed sound.

1. **The write-receipt liveness rule inferred external-effect death from database-session death.** The design and `services/supabaseAdapter.js` made the session-scoped advisory lock the authority for whether another attempt was still live, on the reasoning that "if the owning process dies **or its connection drops**, Postgres releases the lock". **The second half is false.** Postgres releases an advisory lock the instant its connection drops — a network blip, a pooler restart — while the Google Sheets request the attempt is awaiting is an independent HTTP call that can still be in flight and can still commit afterwards. A competitor could take the freed lock and perform the **same append**; the attempt token cannot help, because it only stops the first attempt *acknowledging in Supabase* and cannot un-append a row from Sheets. The existing WRITE-3/P8 tests proved live-lock refusal and lock release, but never the adversarial case, so the whole guarantee family was unproven. **The owner directed replacement, not reconciliation.** Corrected: the reclaim condition is now **owner process identity** (`atlas.write_receipts.owner_instance_id`), which a dropped connection does not change and which mirrors the rule the live file store actually uses — a record is retryable only when it was **rehydrated from a prior process**, never merely because time passed. The lock survives **demoted to intra-process serialisation** and may never again be called a liveness authority. The precondition is stated rather than assumed: "a different instance id means that process is gone" is sound under the **single-instance invariant** §5.3/§5.5 already require, and `S4` may not wire this without it. New gate **P16c** proves the exact adversarial case — terminate the attempt's database backend mid-effect, confirm the lock is genuinely free, age the row past the secondary bound, and show the competitor is still refused — plus the recovery case, so the refusal cannot become a permanent stall.

2. **`docs/ATLAS_SYSTEM_AUTHORITY.md` pre-claimed a state that had not happened.** Concept 18 on that head read `PR S2 landed 2026-08-08 and is DORMANT` while PR #1274 was still open and unmerged. The authority map records **what decides production today** and its own honesty rule rejects a document, a test, or an import as proof of production authority — a branch recording itself as landed is the same error. Corrected: concept 18 states the current live authority as Google Sheets plus the file-backed receipt store, records `S2` as **open and unmerged (proposed, not landed)**, and defers the landed-state update until current `main` contains it. The same pre-claim was swept out of two further surfaces the review did not name — this plan's `PR S2 — DELIVERED` heading and the design document's *"`PR S2` has landed"* status paragraph.

**No counter changed and no authorization was granted by this review.** Campaign 0/5, Stage B 0/5, Phase 5 unauthorized, `SESSION_PLAN_SETS_WRITE_ENABLED` still `0`, no schema applied, no deployment. **The head that fixes both findings requires a new exact-head review; this entry does not carry forward to it.**

### ATLAS CONTRACT / SYSTEMS REVIEW 2026-08-08 (eighteenth required) — exact head `deaa8a513166584449902279ae4063f506ebf553`: BLOCKING

The review confirmed both findings of the seventeenth are materially addressed and CI green on that head, and found **two further P1s in the replacement liveness mechanism**. Both are corrected. The architecture disposition is unchanged, and no phase, count, or write authority was altered by this review.

1. **Process identity fixed connection loss and nothing more.** `owner_instance_id` correctly refuses a competitor after a dropped database session, but a **different instance proves only that the old process is gone** — not that an HTTP request that process already sent to Google Sheets did not commit. Google can accept an append and complete it server-side without the client surviving to record the response, so five minutes of age cannot turn an ambiguous post-send outcome into a proven non-write. This mattered because ruling D4 moves the receipt for all seven callers while the four D4 routes keep their **data** in Sheets, where no Supabase constraint can catch a duplicate. The two liveness proofs each covered one half and **missed the composition**: P16c's connection-loss case keeps the same instance, and its restarted-process case has no unresolved external effect allowed to commit late. **The owner directed effect-awareness, not another timer and not reconciliation after two effects.** Corrected: `atlas.write_receipts.effect_authority` records where each route's authoritative effect lands, declared from a frozen exhaustive route map that **fails closed on an undeclared route**, and it is absent from `atlas_app`'s `UPDATE` grant so the runtime cannot relabel an effect. Automatic reclaim on process death is now confined to `'supabase'` routes, whose effect is one transaction and whose Sheets export is idempotent by an allocated destination (§3.9, §5.3). A `'sheets'` route's abandoned attempt is **never reclaimed**: it moves to the new `'ambiguous'` status — token voided, marked by a compare-and-set safe to race — and only `resolveAmbiguousReceipt`, carrying a **destination-side finding and its proof**, releases it. Both existing timers were closed with it: the claim's expired-row branch and the prune both exclude `'ambiguous'`, and `peekWrite` keeps such a row visible past its TTL so a refusal can say why. New gate **P16d** proves the composition end to end — the dead instance, the refusal recorded as ambiguous, the late commit, and exactly one row at the destination — plus that no timer un-wedges it, that proof releases it in **both** directions, the contrast that a transactional route still reclaims, and the map's fail-closed behaviour.

2. **The new liveness authority was not represented consistently in its own contract.** The migration created `owner_instance_id` and the claim SQL depended on it, while §3.6's canonical table, §3.6's displayed claim statement, and §4.6's record-by-record mapping all omitted it — and the column was declared nullable, so an `in_progress` row with no owner was representable and `IS DISTINCT FROM` would have read that null as "some other process", reclaiming it on age alone. Corrected: all three surfaces now carry the column, §3.6 states its **lifecycle** explicitly (new claim, retry, terminal provenance, ambiguous evidence, `S4` carry), and the rule is enforced **structurally** by `write_receipts_live_attempt_has_owner_check` rather than by the single adapter call site. §4.6 records why `owner_instance_id`, `effect_authority`, and the ambiguity columns **cannot be carried** from a file record, and what happens instead — the `S4` drain normalises every carried `in_progress` record to `failed` before the handover, so no carried row is ever live and no owner is fabricated. Three further constraints make the ambiguous state unable to be entered or left unsoundly.

**No counter changed and no authorization was granted by this review.** Campaign 0/5, Stage B 0/5, Phase 5 unauthorized, `SESSION_PLAN_SETS_WRITE_ENABLED` still `0`, no schema applied, no deployment. **The head that fixes both findings requires a new exact-head review; this entry does not carry forward to it.**

### ATLAS CONTRACT / SYSTEMS REVIEW 2026-08-08 (nineteenth required) — exact head `5533874627293e853822bfec783df4190a088d68`: BLOCKING

The review confirmed both findings of the eighteenth are materially fixed — the `ambiguous` state prevents process death from repeating a non-transactional Sheets effect, neither the TTL nor the prune releases it, destination-side proof is required, `owner_instance_id` is structurally required for `in_progress`, and the design and schema text agree — with exact-head CI green. It found **one new P1**, which is corrected.

**`write_id` was not bound to its route, so a cross-route retry inherited the wrong retry semantics.** `ROUTE_EFFECT_AUTHORITY` declares the seven routes correctly and fails closed on an unknown one, but `route` and `effect_authority` are written only on the initial `INSERT`: the claim's `ON CONFLICT … DO UPDATE` path neither rewrote them nor required the incoming values to match the stored ones, and the primary key is `write_id` alone. A `write_id` first claimed on `/api/log-workout` therefore kept `effect_authority = 'supabase'` when the same id was later presented to `/api/coaching-notes` — through a client bug, a replay bug, or a namespace collision — so that route's non-transactional Sheets append became **automatically reclaimable after five minutes instead of ambiguous**. That is the duplicate class gate P16d exists to make impossible, reached by a different door. The reverse collision only wedges a transactional route; the permissive `supabase → sheets` inheritance is the safety defect, and both are now refused, because a rule binding only the dangerous direction would be a rule about one symptom rather than about identity.

Corrected as directed, as a **state-machine/authority fix rather than another reconciliation path**, and as a **refusal rather than a rewrite** — rewriting `effect_authority` on retry would erase what kind of effect the prior attempt may already have sent. The claim now requires `route = $2 AND effect_authority = $4` outside its three reclaim branches, so a mismatch matches no row; `markReceiptAmbiguous` is bound the same way, since a claimer on a different route must change nothing at all, not even to record an ambiguity about an effect it never sent. `write_id` **stays** the primary key: making `(write_id, route)` the identity would legitimise one `write_id` on two routes, a weaker client contract than exists today. The collision is **observable** — read through a dedicated statement that is not TTL-filtered, because an expired row is the most permissive branch of all, then logged as `write_receipt_route_conflict` and returned with the record **withheld** so no caller can replay a foreign route's response body. Neither column appears in `atlas_app`'s `UPDATE` grant, so the binding cannot be defeated by relabelling a row instead of editing a statement. New gate **P16e** proves the full unsafe sequence refused before any effect with the stored row untouched, the reverse direction, the binding holding in all three reclaimable states rather than only `failed`, the route-bound mark statement, and both grants refused — and a mutation removing the binding fails three behavioural proofs, not only the textual guard.

**No counter changed and no authorization was granted by this review.** Campaign 0/5, Stage B 0/5, Phase 5 unauthorized, `SESSION_PLAN_SETS_WRITE_ENABLED` still `0`, no schema applied, no deployment. **The head that fixes this finding requires a new exact-head review; this entry does not carry forward to it.**

### PR S2 — MERGED 2026-08-08 (#1274): migrations, adapter, shadow write, divergence lane

**HISTORICAL RECORD — the state AS PR #1274 MERGED on 2026-08-08. It is not current state; for current state read "OWNER GATE — `S2` APPLIED AND P8b PASSED" below.** *Every sentence in this section is past tense about that merge. The application clause is **superseded as of 2026-08-08**: the owner has since applied the `S2` schema to `Atlas Production` under owner gate 1(a). What still holds today is that the code remains dormant and no authority has moved.*

**As merged: `main` at `4d3e231`, DORMANT, and not yet applied to `Atlas Production`.** *This landed state was recorded from current `main`, after the merge — not by the branch about itself. The required review of `ad18907` found this heading and the authority map both pre-claiming a merged state while PR #1274 was open; the rule that produced this ordering stands.* The merge changed no running behaviour: the schema existed in `supabase/migrations/` but was applied at that time to **no persistent or hosted Atlas target** — only to the disposable proof database §6.1 P2 creates from empty and destroys with the run — and the shadow lane was unconfigured, so **nothing had migrated**. The **next step was an OWNER GATE**, not a card; **that gate has since been discharged.** `S1` was paper. `S2` is the first executable step of the chain, and it is deliberately inert in production. The gate differs per component and is not one flag: the **request-path shadow lane** needs `ATLAS_SUPABASE_SHADOW_WRITE=1` **and** `ATLAS_SUPABASE_APP_URL`, while the **operator tools** — `atlas:sweep`, `atlas:repair`, `atlas:catalog-sync` — need only their connection string and run with the shadow flag off. What makes all of them inert is simpler than any flag: **no Supabase connection string of any role is configured in any environment.** Google Sheets remains the sole live read and write authority for every migrated concept, and no athlete-facing response, status code, proof field, or visible claim changes.
...
**What `S2` did not do, and must not have.** *Each item below is what PR #1274 itself did not do; the first two were done later by the owner at the gate below.* No schema applied to `Atlas Production`. No Supavisor production-role checkpoint. No athlete-facing read or write moved. No `S3` work. No `atlas.write_freeze`. No counter, Stage B state, Phase 5 authorization, or `SESSION_PLAN_SETS_WRITE_ENABLED` change. No credential or project reference in the repository, a log, a PR body, or an evidence file.

**No counter changed by this PR. Campaign 0/5 · Stage A 5/5 COMPLETE · Stage B 0/5 OPEN. Phase 5 unauthorized. `SESSION_PLAN_SETS_WRITE_ENABLED` stays `0`. No schema applied by this PR. No deployment.**

**The next step was an OWNER GATE, not a card.** Applying `S2`'s migrations to `Atlas Production` was Dale's action. After it, and before `S3` began, the Supavisor four-role session-lock checkpoint (§6.1 P8b) had to pass. **Both are now discharged — the record is immediately below. Nothing above this line describes the current state of `Atlas Production`.**
...
**Effect on authority.** None. Google Sheets through `sheets.js`, plus the file-backed `services/idempotency.js` store for receipts, remains the **sole live authority** for all seven migrated concepts. Applying the schema makes `Atlas Production` a **persistent hosted migration / bridge target** — schema existence on a host that outlives a proof run, **not** a live shadow path. The runtime shadow stays **dormant and unconfigured**, so nothing reaches that schema. **Competing durable authority stays NONE**, and authority transfers only at `S4`.
...
**OWNER SECURITY DECISION 2026-08-09 — the integration is REMOVED, the project reference is ruled NON-SECRET, and the routed item is CLOSED.** Two owner actions on one day, in order.

**1. The integration was removed.** While PR #1279 recorded the gate above, its `Supabase Preview` check target was found to carry the **project reference**. Under the **then-current** §8.4 policy that classified the reference as a secret, so it triggered an **owner security review**; it was routed out of #1279 as `OWNER DECISION REQUIRED` rather than folded in, because the integration **pre-dated** that PR. Dale removed **both** Supabase project↔repository connections from the Supabase organization's Integrations page — **on the grounds that the integration was unnecessary**, not that authentication material had escaped. It had no consumer here, and `P2`'s proof database is a plain disposable Postgres instance that never depended on it (§6.1 P2, ruling D3), so deletion was simpler than retaining unused infrastructure. **Nothing replaced it** — no masking layer, no fallback, no adapter, no reconciliation mechanism — so **net architecture complexity decreased**.

**2. The project reference is ruled NON-SECRET.** **This ruling is the owner authorization; it governs because it is recorded here.** A Supabase **project reference is a non-secret project identifier / metadata** — **not** a password, key, token, or authorization mechanism. **The historical public check targets therefore do not represent a credential compromise and require no rotation, no project replacement, and no remediation.** The ruling changes **only** that classification. It does **not** declassify Postgres or database connection strings, passwords, `ATLAS_SUPABASE_*` role URLs carrying credentials, Supabase secret or service-role keys, OAuth tokens, private keys, the production Sheet ID, or any other separately governed identifier — all of which stay prohibited from the repository, PR bodies, evidence files and GitHub Actions. Design §8.4 carries the rule; the secret scanner's `supabase-project-reference` rule is **deleted with no replacement detector or masking mechanism**, while every credential rule stands and is proven by `test/secret-scan.test.js`.
...
**Status, recorded honestly at PR #1291 authoring time.** PR #1291
(`agent/s4-supabase-cutover`) is the S4 repository cutover. It was not an
authorized production cutover. **At that writing,** `main` was
`da16cd4b13912569870e9a6dee7a3281730027b1` and still had Google Sheets as the live
workout authority and `services/idempotency.js` as the live receipt store. The
branch wired Supabase as the intended authority for the seven hot-path concepts,
...
application, cutover, or data write has occurred. **That last sentence is the
PR's merge-authorization state. It is not current production state. Render
later served the merged head. See the accidental-deploy recovery block below.**
...
unauthorized. No schema applied. **That paragraph is the state as of the
PR #1291 merge authorization.** It is not current production state. Current
production state is the accidental-deploy recovery block immediately below.
...
2. **The seven write routes are not closed by mechanism after S3 restore.**
   ...
   and every agent must not call them. That is an operational hold, not a new
   control.
3. **S3 receipt state is destroyed and unprovable.** The S4 process replaced
   the S3 process and deleted `services/idempotency.js`.
   `/tmp/atlas-idempotency.json` is gone. The designed reverse transfer cannot
   run. This recovery does not invent a substitute carry.
...
5. **No schema reversal.** S4 schema apply (gate 1(c)) is outstanding. This
   ...
   shows any of `20260813152939_s4_catalog_sole_authority`,
...
line already records.** Phase 5 stays unauthorized. It authorizes no schema
application, no freeze, no production write, no write reopening, and no S4
cutover.
