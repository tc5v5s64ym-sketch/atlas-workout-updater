...
### OWNER AUTHORIZATION 2026-08-02 — F-SB4B option A: the two sandbox ledger tabs (no phase change)

**This authorization governs because it is recorded here.** Dale answered the F-SB4B preflight decision recorded above and selected **option A**.

**What is authorized.** Creating `Session_Plans` and `Session_Plan_Sets` in the **declared sandbox workbook only** (`config/sandboxSheet.js`, last6 `H3CeXE`), using the canonical headers in `config/columns.js` — `sessionPlansColumns` (13) and `sessionPlanSetsColumns` (16), in that exact order.

**Architectural constraints, as dictated:**

1. This is a **temporary compatibility bridge**, and its only purpose is to complete the F-SB4 owner-pattern rehearsal.
2. **No production workbook schema change** is authorized.
3. **No production data may be written.**
4. `SESSION_PLAN_SETS_WRITE_ENABLED` on Render **stays unchanged**.
5. The **sandbox workbook is the only workbook** this authorization covers.
6. **No broader schema-creation capability and no permanent fallback** may be introduced.
7. The gate-harness exception must be **explicitly temporary, documented, and removed in F-SB4C** immediately after the rehearsal completes.
8. After F-SB4C the sole authority returns to the **normal production schema path**, with **no sandbox-specific exception remaining**.

**Exact sunset condition.** Every artifact below is deleted in the F-SB4C completion PR, and its absence is verified there:

- the sandbox-only ledger-tab provisioning helper and its operator entry point;
- the gate harness's temporary permission to combine `ATLAS_GATE_SANDBOX_LIVE=1` with `ATLAS_GATE_LEDGER_SANDBOX=1`, restoring the unconditional refusal;
- the restored sandbox-live posture and every other temporary rehearsal artifact already listed under F-SB4C.

The two tabs themselves live in the sandbox workbook, not in the repository. They are outside the tree, so F-SB4C records them as sandbox residue rather than deleting them from code.

**What this authorization does NOT do.** It does not reopen Stage A, is not Stage B evidence, and cannot advance Stage B. **Stage A: 5/5 — COMPLETE. Stage B: 0/5 — OPEN. Rehearsal (F-SB4): 0/5.** Phase 5 stays unauthorized regardless of the rehearsal's outcome. A meaningful failure after the first synthetic athlete turn resets the **rehearsal** streak to 0/5 under the rule already recorded above; the Stage B counter is not touched by any agent-run session.
