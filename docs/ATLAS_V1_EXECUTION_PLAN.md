...
`CAMPAIGN STATE: Phase 2 — Inventory, contracts, and clean paper. Next step: SESSION 2 — Batch B, part 2 of 2 = Drift Guard 4 (the completion-ladder manifest, part 1 of 2, ✅ DONE 2026-07-21; SESSION 1 ✅ DONE 2026-07-20). The two owner-sequenced FRESH SESSIONS (owner instruction 2026-07-20): SESSION 1 ✅ DONE 2026-07-20 = BACKLOG archive reconciliation + Guard 6's staleness + auto-archive machinery. RECONCILE (per-item pass over all 1263 lines against docs/BUG_TRIAGE_LEDGER.md + the plan + git; NO whole item was safely movable — every remaining ✅ carries an open follow-up, anchors a cross-ref, sits in a live [ ]/[x] checklist, or belongs to an active lane; the clean self-contained items already went to BACKLOG_ARCHIVE.md in PR-21 — recorded in docs/verification/BACKLOG_RECONCILIATION_2026-07-20.md; cap stays 1263, the honest floor). GUARD 6 COMPLETED: staleness check (CI fails when an [archive-ready: DATE] tag is >7 days old, config/paper-weight.json stale_after_days) + the auto-archive job (scripts/archive-backlog.js, `npm run archive:backlog` moves tagged items to BACKLOG_ARCHIVE.md and ratchets the cap), both gated on the explicit tag — never free-text heuristics, which the reconciliation proved can't safely tell a fully-shipped item from a ✅-with-open-follow-up one. Hardened 2026-07-20 (Codex review): the auto-archive job never RAISES the cap (grow-only guard preserved even when the file is over cap), and any malformed [archive-ready] date fails closed. SESSION 2 = Batch B — the completion-ladder manifest (item 3) + Drift Guard 4 — under the recorded ladder rulings (lower rung when ambiguous; owner-accepted ONLY via an owner gate; publish the ladder table). BATCH B PART 1 ✅ DONE 2026-07-21: the manifest's single complete/partial/missing status is retired; every capability in config/coaching/manifests/capabilities.json now carries the nine-rung `ladder` (built, unit_tested, runner_wired, inputs_available, route_consumed, user_visible, validator_covered, live_proven, owner_accepted), assessed against the Work-item-1b inventory + real code evidence (6 reach inputs_available, 10 unit_tested, 3 not-built; ZERO reach route_consumed — the One-Brain orchestrator is shadow/flag-gated, route-consumption is Phase 4). validateManifest enforces shape+monotonicity+name-the-consumer; owner_accepted is owner-gate-only; the skimmable table is published at docs/CAPABILITY_COMPLETION_LADDER.md. NOW NEXT = Batch B part 2 = Drift Guard 4 (scripts/check-completion-ladder.js, npm run check:ladder): no capability may claim route_consumed or live_proven without a linked test or trace id. DONE: all EIGHT canonical contracts ratified read-only; batch A (paper-hygiene safe parts — app.js freeze rule, closeout label, BACKLOG dating — + Drift Guards 1 authority / 2 banned-pattern / 3 wiring-hardened / 6 paper-weight (size cap + staleness + auto-archive), all merged and green in CI) + the reconciled open-P0/P1 index at the top of BACKLOG + the whole-item BACKLOG archive reconciliation (Session 1). Earlier: Work item 1 (1a semantic wiring guard, 1b ownership/connectivity inventory); Work item 2 charter (docs/CANONICAL_CONTRACTS.md); contracts staged in wiring-allowlist until Phase 3/4/5b/5d/5e/5g (CoachingDecision already ratified). Gates passed: install, Phase 1 Soul gate (2026-07-20 PASS). Streak: 0/5.`
