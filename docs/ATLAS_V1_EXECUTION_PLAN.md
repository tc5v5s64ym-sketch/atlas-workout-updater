...
Structured chat proposals use the same initiation authority: after prose rendering and immediately before each edit, plan edit, note, or constraint side effect, the client rechecks that the accepted response ticket still owns the session. Beginning a newer preview or response synchronously revokes that authority, so stale structured output cannot mutate or clear the retained preview.
