...
The schema is **eleven tables: ten permanent and one temporary**. The eleventh, `atlas.migration_divergences`, is migration-control machinery rather than product data, and `S4` drops it. Five tables beyond the original six were added by owner ruling or owner review, never by scope drift: the `Exercise_Catalog` read mirror and its separate freshness authority `exercise_catalog_sync` (ruling D1, split by the second review), the divergence table the first review required, and the export destination authority `sheets_mirror_cursor` + `sheets_mirror_allocations` the fourth review required. **None widens the migrated workout data** — every one is control or reference machinery for the migration itself.
