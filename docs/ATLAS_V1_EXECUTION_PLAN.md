...
*Exact stopping point:* F10 is **not started** — no F10 code change. F09A–F09J complete + merged; the F10 ledger design (`docs/SESSION_PLANS_LEDGER_DESIGN.md`) approved with the three owner amendments + merged (#1050); worktree clean. Next actionable step = introduce `planSlotStatuses` red-first and route the surfaces above through it (satisfying AC "one logged variant completes exactly the intended `plan_item_id`; duplicate names stay slot-distinct; exact outranks substring; recap/next-up/pin/handoff/closeout/Workout Sheet agree from one selector"), then proceed F10A–F10C, stopping at the F10D production-tab/write gate.
