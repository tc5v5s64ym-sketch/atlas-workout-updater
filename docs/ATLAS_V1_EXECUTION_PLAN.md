...
**Status:** SLICE 1 COMPLETE — acceptance capture + revision infrastructure shipped. Slice 2 (mid-session explicit-revision **triggers** + reload reconstruction) tracked as `BACKLOG.md` `F10B-REVISION-WIRING-1`.
...
**Remaining (slice 2, `F10B-REVISION-WIRING-1`).** Wire the mid-session revision TRIGGERS to `POST /api/session-plan-sets/revision`: an explicit substitution/pivot forms a new recommendation before the substitute's work; an explicit post-set Atlas recommendation appends a future-only revision (guarding the frozen-performed-set invariant at the call site); a user change becomes a revision only when Atlas endorses it. Plus the reload/resume reconstruction of session state from the durable ledger. Touches coach-conversation/substitution (higher-risk) — a focused follow-up.

**Completion record:** PR (slice 1) — this PR (F10B) · Commit — new `services/sessionPlanSetsCapture.js`; `routes/sessionPlans.js` + `config/routes.js` (accept/revision routes); `src/app/planAcceptance.js` (`buildLedgerAcceptedItems` + checkpoint post) + `src/app/app.js` (`postLedgerCheckpoint` dep); `config/wiring-allowlist.json` (ledger services now wired); tests `test/sessionPlanSetsRoutes.test.js`, `test/sessionPlanSetsCapture.test.js`, `test/planAcceptance.test.js` (F10B additions). Full suite 5651 pass; lint 0 errors.
