...
Structured chat proposals use the same initiation authority: after prose rendering and immediately before each edit, plan edit, note, or constraint side effect, the client rechecks that the accepted response ticket is still the single global owner of the shared UI. Beginning a newer preview or response synchronously revokes that authority across sessions, so stale structured output cannot mutate or clear the retained preview after the user has moved from S1 to S2. A response that cannot mint a bounded ticket (including a malformed or oversized session id) has no structured-effect authority; missing authority fails closed instead of falling back to permissive behavior. The held-clarification affirmation path binds its resolved rows to the current logging session before emitting `atlas:set-logged`, so the resulting `/api/coach/message` turn replaces any older turn in that same session and is the turn carried through closeout preview and approval.
