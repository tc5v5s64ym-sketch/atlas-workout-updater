...
**Bottom line (24 rows, updated 2026-07-02 PT):** 16 shipped, **5 fixed pending an owner live re-test**
(the coach-fallback pair -153258/-153312, the recovery-bump recurrence -204817 — which FAILED its
07-02 live re-test and is re-fixed by the 07-02 chat-report row (route `?intentId` passthrough +
LLM `session_intent`), the restore banner -003505, the knee-raise prompt -002945), **0 open**, 4
noise (one a near-dup). All known-actionable rows now have a shipped fix; the remaining work is
owner live validation — see the live-test items in `BACKLOG.md` / the QA campaign.
