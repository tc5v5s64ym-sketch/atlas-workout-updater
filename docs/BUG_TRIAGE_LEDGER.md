...
|---|---|---|---|---|---|---|
| BUG-20260629-204817 | 06-29 13:48 | "Coaches pick called for a recovery session then called me out on not lifting enough" | **Recurrence of -034034.** A recovery-INTENT Coach's Pick (`recovery_pump` / `deload_reset`) still nudged add-load via TWO voices: the per-set "Too much left in the tank. Bump coming." (`effort_note`) AND the "Next time: move to 175 × 9" (`recommendNextSet`). #704 fixed the first but sourced the intent from `activePlannedSession.intentId`, which is **null** for an engaged-but-unmaterialized pick — so live (shell v77) it still fired (owner repro 06-29). | 🟡 fixed, live re-test | #704 (partial) **+ this PR** — `getActiveIntentId()` now falls back to the engaged Coach's Pick intent, so both the set-reaction AND the recommend call receive it; `recommendNextSet` also holds the weight on a recovery intent (no "move to X"). | this lane |
