...
This document defines how Dale, ChatGPT, the approved active implementation agent, GitHub, and the repository execute Atlas work. `CLAUDE.md` is the canonical detailed rulebook and carries the definition of that role; `docs/BUILDER_PORTABILITY.md` maps its legacy tool-specific wording; `docs/ATLAS_V1_EXECUTION_PLAN.md` selects the work.
...
Atlas's architectural owner and project decision desk. It resolves genuinely non-derivable product/scope/trust forks with Dale, and it performs the required Atlas Contract / Systems Review. A clean-context review by the implementation agent is advisory only and never satisfies that gate. `CLAUDE.md` holds the one trigger list.

### The approved active implementation agent

The approved active implementation agent runs the current-state gate, implements one card/concern on a fresh branch, tests it, opens the PR, declares its builder surface and model in the merge card, addresses real in-scope advisory findings, merges the exact passing head under standing authority, updates campaign state when required, refreshes `main`, and continues.

The role is held by whichever agent Dale approves, on whichever surface it runs — Claude Code, Codex, Cursor, or another owner-approved surface. `CLAUDE.md` carries the canonical definition. Every holder of the role has identical implementation and merge authority. Surface identity and model identity never change an owner-reserved gate. See `docs/BUILDER_PORTABILITY.md` for the legacy wording map, the switch protocol, and the cold-start acceptance trial.
...
Independent agent review—any agent that is not the active builder, or another clean-context review—is advisory only. The active builder fixes confident, real, in-scope findings; files non-blocking future work when authorized; and records false alarms without creating a synthetic status gate. An independent builder-model review never substitutes for a required deterministic gate or for the Atlas Contract / Systems Review.
...
3. Create a fresh `agent/<concern>` branch from current `main`, on every surface. Existing `claude/*` branches remain valid historical branches but are not the form for new work.
...
7. Push and open one PR with the Atlas Merge Card, its four attribution fields, and one primary risk label.
...
- Never run two implementation agents on the same concern in parallel.
...
Use `docs/BUILDER_PORTABILITY.md`. The default is to switch only after the current concern is merged or explicitly abandoned. Repository state, not a custom handoff prompt, carries the work. A fresh agent on a new surface proves that with the cold-start acceptance trial recorded there.

## Compact launcher

`AGENTS.md` carries the one compact launcher, for every surface. Do not write a second one.
