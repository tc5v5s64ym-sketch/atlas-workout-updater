...
This document defines how Dale, ChatGPT, Claude Code, GitHub, and the repository execute Atlas work. `CLAUDE.md` is canonical; `docs/ATLAS_V1_EXECUTION_PLAN.md` selects the work.
...
5. this workflow and the relevant specs/invariants/tests

Historical plans, audits, proposals, and Git history never select the next PR.
...
Runs the current-state gate, implements one card/concern on a fresh branch, tests it, opens the PR, addresses real in-scope advisory findings, merges the exact passing head under standing authority, updates campaign state, refreshes `main`, and continues.

### GitHub and Codex

GitHub Actions supplies deterministic hard gates. A missing, stale, skipped, errored, timed-out, or failed required check is a failure.

Codex review comments are advisory only. Claude fixes confident, real, in-scope findings; files non-blocking future work when authorized; and records false alarms without creating a synthetic status gate.

## Standard campaign loop

1. Fetch/verify current `main`, confirm the worktree is clean, and identify the first eligible unfinished card in `docs/ATLAS_V1_EXECUTION_PLAN.md`.
2. Run the Current-State Verification Gate before editing.
3. Create a fresh `claude/<concern>` or `agent/<concern>` branch from current `main`.
...
9. Address real in-scope Codex/advisory findings. Do not widen the PR.
...
A long or exhausted chat session is not a blocker. The repository is the handoff: start a fresh session and resume from the canonical plan.
...
- New work begins on `claude/*` or `agent/*` from current `main`.
...
Claude may merge when:

- every applicable required GitHub check passed;
- real Codex/advisory findings are fixed or explicitly judged non-issues;
...
A clean-context review is optional confidence work, not a required marker or separate account.
...
A valid fresh-session prompt is:
