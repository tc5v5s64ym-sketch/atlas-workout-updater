...
> The current active lane is the **required Soul work** in [`docs/SOUL_PLAN_V1.md`](./SOUL_PLAN_V1.md) — see its **"Soul completion & Post-Soul handoff contract"** for the required exit criteria and what remains open (as of 2026-07-12: **S2** — structured-goal production setup (owner supplies Bench/Squat/bodyweight/consistency/deadline goals → approved rows added to the production `Constraints` surface → Atlas live-reads/forwards them, no invented data); **S5** — finish the engine-granted voice + record **LT-010** (profanity OFF); **S7** — record **LT-011**). Until every required Soul criterion is recorded complete, Soul retains execution priority.
