...
> The **One-Brain promotion clock** (GATE A → PR-A5 → burn-in → PR-12B; the "Active queue" below) runs **in parallel** on its own owner-reserved evidence window (`docs/ONE_BRAIN_PROMOTION_CRITERIA.md`); whether promotion must land before Soul is *declared* complete is the one open owner question flagged in the Soul completion contract.
