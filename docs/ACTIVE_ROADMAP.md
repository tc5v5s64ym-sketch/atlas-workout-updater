...
> **▶ Phase sequence (2026-07-12) — Soul now, Post-Soul finishing next, then V1.**
> The current active lane is the **required Soul work** in [`docs/SOUL_PLAN_V1.md`](./SOUL_PLAN_V1.md) — see its **"Soul completion & Post-Soul handoff contract"** for the required exit criteria. **Only ONE required criterion remains (as of 2026-07-12): S5** — finish the engine-granted voice + record **LT-010** (profanity OFF). **S2** was resolved via the owner-confirmed **no-active-goals** path (empty-goals verified safe, no invented goals); **S7** is closed — **LT-011 re-validated live to PASS** against the deployed reassure-voice fix (`e94bfbb`). Until S5 is recorded complete, Soul retains execution priority; once it is, the Post-Soul handoff fires.
> The **Post-Soul V1 Finishing Plan** — [`docs/POST_SOUL_V1_FINISHING_PLAN.md`](./POST_SOUL_V1_FINISHING_PLAN.md) — is **adopted and queued next** (POST-01…POST-12: close remaining P0/P1 trust risks → cross-seam proving packs → five clean live sessions → declare V1 → two-week defect-only stabilization). Per the handoff rule it becomes the governed next phase **automatically via document-driven work selection** once Soul's required criteria are complete — **no owner re-prompt, and no trigger/reminder/scheduled job.**
> The **One-Brain promotion clock** (GATE A → PR-A5 → burn-in → PR-12B; the "Active queue" below) runs **in parallel** on its own owner-reserved evidence window (`docs/ONE_BRAIN_PROMOTION_CRITERIA.md`). **Owner decision (2026-07-12): promotion is NOT a blocking Soul-completion criterion** — Soul completes on its required voice/persona criteria + live validations (S1–S7), and the Post-Soul plan may activate with GATE A still open (it carries GATE A visibly as an unresolved owner gate). GATE A stays owner-reserved and **never promotes automatically**; it continues to block only capabilities whose spec explicitly requires promotion (e.g. PR-B6 Moments, PR-12B legacy deletion).
> **This is one sequence, not a second roadmap.** `docs/SOUL_PLAN_V1.md` (the active lane) and `docs/POST_SOUL_V1_FINISHING_PLAN.md` (queued next) are the ordered lanes; `BACKLOG.md` remains the single source of truth; this file stays the detailed active queue. No competing priority system is created.
...
> **Vision-first selection.** This queue (and any `BACKLOG.md` refill) is subordinate to the **north star**: the **Vision** (`docs/ATLAS_PRODUCT_VISION.md`) is the product direction, the **Roadmap layer** is the sequencing map — `docs/ROADMAP.md` holds directional milestones (reference only; this file + `BACKLOG.md` are the live sequence) — and the **Architecture** (`docs/ARCHITECTURE.md`) is the system boundary. Before selecting or building, read the north-star context order and run the **Vision Alignment Check** in `docs/AGENT_WORKFLOW.md` ("North-star context — read FIRST"). A queue item is eligible only if it clearly advances the Vision and respects the Roadmap/Architecture; if the highest-priority item does not, **stop and report the conflict** rather than ship it (`docs/OWNER_CHECKIN_RULES.md` category 4, Escalation Policy v3).

**Current active build (updated 2026-07-12): the required Soul work (`docs/SOUL_PLAN_V1.md`), with the One-Brain coaching engine promotion running as the parallel evidence clock** — see the Phase-sequence banner above. *(History: the Composer-First surface migration was owner-adopted 2026-07-02 and has fully shipped; the paragraph below is retained as the origin record for the One-Brain lane.)* The composer-first surface migration — design + phase sequence in [`COMPOSER_FIRST_MIGRATION.md`](./COMPOSER_FIRST_MIGRATION.md); build queue in `BACKLOG.md` → "Order of the day" item 1. The One-Brain engine lane (blueprint [`COACHING_ENGINE_ARCHITECTURE.md`](./COACHING_ENGINE_ARCHITECTURE.md); sequence in `BACKLOG.md` → "One-Brain Coaching Engine") continues as that migration's Phase 0 engine work, under Invariant I1 (*the conversation is the product*). The earlier "Trust-Critical Coach Interaction Layer (P0–P4)" workstream this header named has been addressed; its diagnosis record is retained below as history. **Still owner-gated inside the adopted plan:** the Phase C input-LLM provider/model decision, and each Phase B/C/D start (see [`CONVERSATION_FIRST_DESIGN_REVIEW.md`](./CONVERSATION_FIRST_DESIGN_REVIEW.md) adoption addendum).
...
> **Status (2026-07-01): retained as diagnosis history — not the current workstream.** The live-testing trust failure this section diagnosed has been addressed (P0 active-session context integrity shipped). The current active build is the **One-Brain coaching engine** (see the header at the top of this file and `BACKLOG.md` → "One-Brain Coaching Engine"). This section is preserved as the root-cause record for the Trust-Critical lane.
...
- **Active queue status (updated 2026-07-01): the live build is the One-Brain coaching engine sequence** in `BACKLOG.md` → "One-Brain Coaching Engine" (owner-promoted, in progress). The full 477–486 Training Intelligence series, Active Workout State Unification (PR1–PR7), and substitute-prescription loadability (#630) are all shipped. Conversation-first *surface* changes (demoting dashboards/tiles to conversation artifacts) remain **owner-reserved** — do not promote them without owner approval (`docs/CONVERSATION_FIRST_DESIGN_REVIEW.md`).
