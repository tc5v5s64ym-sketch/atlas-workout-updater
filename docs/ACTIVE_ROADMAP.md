...
**Current active build: the One-Brain coaching engine** — blueprint in [`COACHING_ENGINE_ARCHITECTURE.md`](./COACHING_ENGINE_ARCHITECTURE.md); the live, owner-promoted build sequence lives in `BACKLOG.md` → "One-Brain Coaching Engine". It is being built under the **conversation-first** coach direction (Invariant I1 — *the conversation is the product*). The earlier "Trust-Critical Coach Interaction Layer (P0–P4)" workstream that this header named has been addressed (P0 active-session context integrity shipped); its diagnosis record is retained below as history. **Scope note:** the One-Brain *engine* build is owner-promoted and active; adopting the conversation-first *Vision* and demoting existing UI surfaces to conversation artifacts is **owner-reserved** and not yet approved (see [`CONVERSATION_FIRST_DESIGN_REVIEW.md`](./CONVERSATION_FIRST_DESIGN_REVIEW.md) and [`CONVERSATION_CONTRACT_V1.md`](./CONVERSATION_CONTRACT_V1.md)).
