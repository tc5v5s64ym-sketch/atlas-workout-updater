...
**Mechanics.** The derivation calls the same deterministic engine an explicit ask would, over pre-session history with the current session excluded (rule 4); it accepts the engine's output as a target **only** if weight, reps, and RIR are each an exact scalar (rule 3) and neither reps nor RIR is a range (rule 2) — otherwise `no_reliable_target` (rule 5). **When the target is reliable**, one `implicit_unplanned` row (`plan_version 1`, `set_index 1`, `target_set_count 1`, `supersedes_key` empty, `confidence` `reliable`, bodyweight `0` preserved) is snapshotted **before** the just-logged result is incorporated. **When `no_reliable_target`**, Atlas displays it but appends **no** ledger row (rule 5) — unlike an accepted-plan slot, an unannounced exercise with no confident target has no recommendation to record. The actual sets are recorded separately in `Log_Cleaned` as always (rule 7). No performed value ever writes or revises this row.
