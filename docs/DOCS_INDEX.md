...
- `docs/AUTOMATION_PROTOCOL.md` — the automation contract: Codex implementation, separate native correctness/security and ChatGPT contract-review lanes, positive-evidence gates, no GitHub Actions-manufactured native review approval, and Dale-only merge authority.
...
- `.github/PULL_REQUEST_TEMPLATE.md` — the one-screen merge card every PR must produce, including exact-head native Codex review evidence.
...
- `COACH_PERSONALITY.md` — how the coach _feels_ (earned reactions, tone). Active voice spec.
