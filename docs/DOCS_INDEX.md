...
- `docs/SESSION_PLANNING_ENGINE.md` — planning spec for the Session Planning Engine (daily objective scoring, supported session objectives, deterministic exercise order rules, fatigue-adjusted plans, golden-fixture proposal). Builds on the Training Profile Taxonomy. Owner-gated, not active roadmap; filed in `BACKLOG.md`.
