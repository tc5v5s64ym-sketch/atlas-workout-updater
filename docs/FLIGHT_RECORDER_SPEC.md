...
- **Session linkage** *(live-validation follow-up, shipped)* — server `api_response` events are stitched to the client transcript: the client sends `x-atlas-flight-session` / `x-atlas-device-id` on every app `/api` call (via `flightRecorder.requestHeaders()` spread into `app.js`'s `api()`; `{}` ⇒ byte-identical when off), the middleware stamps them, and `recordApiFlow` derives a server-side **per-session `seq`** (independent monotonic counter, blank when unlinked) so server events are orderable within a session. Previously those three fields were blank.
...
- **`POST /api/flight/ingest`** *(PR-FR3, shipped)* — the CLIENT batch sink for the UI-only events the server can't see. `writeCapable:true`, rate-limited, `recordClientBatch` appends the whole batch in **one** best-effort call to `Flight_Recorder` on the server's own sheet (never a workout/trust tab), **202 no-op when the flag is off** so a stale client can't force a write, same sim→non-sandbox isolation guard.
- **Shadow linkage enrichment** *(deferred — FR follow-up)* — the response envelope carries a tiny additive `_flight.shadow` block the client stamps onto its event, riding the existing `standardSuccess` builder (no per-route surgery). Deferred from FR3 to keep that slice to capture + ingest.
