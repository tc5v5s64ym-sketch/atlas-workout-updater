...
**Status (One-Brain Promotion PR 1 + `/api/plan/today` follow-up, shipped):** `brian` is implemented for **`GET /api/recommend/next/:liftCode`** and **`GET /api/plan/today`** — `services/coachEnginePromotion.js` (`planBrianOverride`/`applyBrianOverride`), the latter looped per lift already selected for the dashboard card, with snapshot/deload state assembled once and shared across lifts. When a valid, `answered` `progression` decision exists, its `target_weight`/`target_reps` drive `next_target`/`recommendation`/`reasoning`; every other legacy field (`next_target.sets`, `target_rir`, `deload`, `rule_decision`, etc.) is untouched. Any error, invalid decision, `clarification_needed`, or an **active deload** (the predefined protocol always wins — a progression decision has no deload awareness) falls back to the legacy response unmodified. `recommendation.engine_source = { mode, driven_by, reason? }` records which path produced the response; this field only ever appears in `brian` mode. **Update (2026-07-02 / 2026-07-05):** the `scoreIntents`-based "Coach's Pick" card (`/api/plan/intent-recommendation`) was promoted to the same gated pattern (`planBrianPickOverride`/`applyBrianPickOverride`) — and then gated harder: the **Brian serve-eligibility rail** (PR #849, `isBrianServeEligible` in `services/coachEnginePromotion.js`) allowlists only the `progression` decision type, so in `brian` mode a `workout` decision falls back to legacy (`reason:'not_serve_eligible'`) while `hybrid` still composes and observes the full pick. Flipping `workout` to serve-eligible is an owner-gated promotion under `docs/ONE_BRAIN_PROMOTION_CRITERIA.md`. Hybrid observability shipped as `services/brainShadow.js` (every orchestration recorded — wins, declines, crashes — plus the optional durable `Brain_Shadow` tab). Coach chat/message and the in-session set reaction remain unpromoted — see `BACKLOG.md`.
