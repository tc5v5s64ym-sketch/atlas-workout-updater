...
function looksLikeSessionRequest(text) {
  ...
  // F-SB1-C (Stage B workout 1, 2026-08-01): a LIFT-LEVEL swap is not a request to generate
  // today's session. "Bench press is taken at the moment, suggest a substitute workout out
  // with reps and weights" matched the generic `(suggest|recommend) a <word> workout` shape
  // below — its optional adjective slot took "substitute" — so the owner asking for a bench
  // alternative had today's plan read back at him.
  //
  // The substitution word ALONE is not enough to decide (Codex P2, PR #1238): "recommend an
  // alternative workout" is a genuine whole-session request and must keep its lane. What
  // separates them is whether the message points at a particular lift — by naming the swap
  // target ("a substitute FOR bench press") or by reporting why that lift is unavailable
  // ("bench press is TAKEN"). A whole-session alternative does neither. "instead of X" always
