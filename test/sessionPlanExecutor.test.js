...
test('app.js next-up follows the visible plan order (started OR coach-suggested) minus completed', () => {
  ...
  // nextPlanned + plannedQueue derive from one shared remaining-after-this-log helper.
  assert.match(src, /const remaining = remainingPlannedExercises\(\);/, 'nextPlanned derives from the shared remaining helper');
  assert.match(src, /const nextPlanned = remaining\[0\] \|\| null;/, 'nextPlanned is the first remaining planned exercise');
  ...
  assert.match(src, /function remainingPlannedExercises\(\)[\s\S]*?remainingSlotNames\(/, 'remaining routes through the authoritative plan_item_id-keyed slot selector (F10)');
