...
test('bug report UI has settings trigger and failure copy fallback', () => {
  ...
  assert.match(sw, /atlas-shell-v95/, 'bug report UI wiring changes must bump the service worker cache');
});
...
test('shell cache: service worker version bumped and all shell scripts precached', () => {
  const sw = fs.readFileSync(path.join(repoRoot, 'public', 'sw.js'), 'utf8');
  assert.match(sw, /atlas-shell-v95/, 'cache name must be bumped so stale assets are evicted');
  assert.doesNotMatch(sw, /atlas-shell-v94\b/, 'old cache name must be gone');
