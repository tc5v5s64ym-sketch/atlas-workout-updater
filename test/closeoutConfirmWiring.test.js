...
test('F10D screenshot conversion: the closeout payload uses the screenshot-resolved identity', () => {
  // Screenshot-resolved id → the established field value → blank. `sessionId` is a
  // local PROVISIONAL value for correlation and must never reach a write payload as
  // though it were a decided identity; blank means "server, you allocate".
  assert.match(appSrc, /session_id: screenshotResolvedSessionId \|\| explicitSessionId,/,
    'the payload session_id prefers the screenshot-resolved id, then the established field');
  assert.doesNotMatch(appSrc, /session_id: screenshotResolvedSessionId \|\| sessionId,/,
    'the provisional client-derived id never becomes the payload identity');
