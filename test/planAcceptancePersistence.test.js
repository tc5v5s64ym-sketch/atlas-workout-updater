...
const ACCEPTED_PLAN = {
  ...
  exercises: [
    { name: 'Bench Press', liftCode: 'BEN01', plan_item_id: 'pi_aaaa1111' },
    { name: 'Barbell Row', liftCode: 'ROW01', plan_item_id: 'pi_bbbb2222' },
  ],
  index: 0,
};
...
test('an accepted plan-only snapshot (no sets) survives reload with pv_/pi_ identity intact', () => {
  ...
  // PR-G1: the execution-view identity tag also survives reload (skip/substitute
  // after a resume still resolves the accepted item by plan_item_id).
  assert.deepEqual(restored.exercises.map(e => e.plan_item_id), ['pi_aaaa1111', 'pi_bbbb2222']);
});
