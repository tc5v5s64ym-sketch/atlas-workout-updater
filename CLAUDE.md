...
2. **REJECT** — not required, too speculative, a duplicate, not worth its complexity, or outside Atlas's intended product. Record the rejection and rationale in the PR discussion. Do not add it to `BACKLOG.md` and do not open an issue.
3. **FILE AS GITHUB ISSUE** — real and worth building, but not this PR. Open a dedicated issue stating user-visible impact, severity, concrete acceptance criteria, and triage status. A target phase or date is not required until the issue is actually prioritized.

Only a prioritized issue enters `docs/ATLAS_V1_EXECUTION_PLAN.md`. "Add to BACKLOG" and "defer to BACKLOG" are prohibited. **Advisory review findings do not automatically become work** — each one is fixed, rejected, or filed. A finding that is not worth filing is **rejected, not stored**. Record the outcome in the merge card's "Additional findings" field. One PR equals one concern is unchanged: a disposition never expands the current PR.
