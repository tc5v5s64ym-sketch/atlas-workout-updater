...
- **Guard 7 — Backlog intake closure** (`scripts/check-backlog-intake.js`, `npm run check:backlog-intake`): the 2026-07-30 owner ruling made `BACKLOG.md` a frozen legacy inventory. On a PR diff (base versus head) CI fails when `BACKLOG.md` gains a work item or a line, or when `backlog_max_lines` rises above the base value. It is mechanical — three counts, no prose classification — so removal, archival, deduplication, and in-place correction all pass, and no justification text can turn it green. Synthetic-violation self-tests: `test/backlogIntakeGuard.test.js`. Known limit: deleting one item and adding a different one keeps every count flat; review and the merge card's "Additional findings" field cover that case.
...
### Finding disposition (owner ruling 2026-07-30 — backlog intake closed)

`BACKLOG.md` is a **frozen legacy inventory, not an intake destination**. Every newly discovered issue, review finding, follow-up, edge case, or improvement takes exactly one disposition:

1. **FIX NOW** — a genuine blocker or an in-scope correctness defect. Fix and test it in the current PR.
2. **REJECT** — not required, too speculative, a duplicate, not worth its complexity, or outside Atlas's intended product. Record the rejection and a short rationale in the PR discussion. Do not add it to `BACKLOG.md` and do not open an issue.
3. **OWNER DECISION REQUIRED** — use only when the finding needs a genuine product, safety, scope, architecture, or priority decision. Stop and report it to the owner. Do not open a GitHub issue, do not add it to `BACKLOG.md`, and do not add it to `docs/ATLAS_V1_EXECUTION_PLAN.md` unless the owner explicitly selects it.

GitHub Issues are **not** a replacement backlog (owner correction 2026-07-31). Work enters `docs/ATLAS_V1_EXECUTION_PLAN.md` only when the owner selects it. "Add to BACKLOG", "defer to BACKLOG", and "file it as an issue" are all prohibited dispositions. **Advisory review findings do not automatically become work** — each one is fixed, rejected, or escalated to the owner, and **agents may not manufacture future work queues from advisory findings**. A finding not worth an owner decision is **rejected, not stored**. Record the outcome in the merge card's "Additional findings" field. One PR equals one concern is unchanged: a disposition never expands the current PR.

`BACKLOG.md` receives no new work items and may only be corrected, deduplicated, archived, or reduced. `config/paper-weight.json` `backlog_max_lines` is permanently non-increasing — never raise it to make CI pass. Drift Guard 7 enforces both mechanically.
