...
- **Guard 6 — Paper-weight** (`scripts/check-paper-weight.js`, `npm run check:paper-weight`): `BACKLOG.md` may not exceed `config/paper-weight.json` `backlog_max_lines` — a shrink-only size cap that ratchets down as the file is trimmed. The staleness sub-rule (shipped items > 7 days) and the auto-archive job are deferred to the backlog archive editorial pass (they need the same per-item shipped-detection the file's header says a mechanical pass can't safely do).
