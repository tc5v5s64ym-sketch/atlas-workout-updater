...
async function mockApis(page, capture) {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (path === '/api/log-workout' && method === 'POST') {
      ...
      return route.fulfill(json({ status: 'success', data: { sheet_write: 'success', sheet_written: true, log_rows_written: (body.log_rows || []).length, logAppendedRange: 'Log_Cleaned!A200:L204' } }));
    }
