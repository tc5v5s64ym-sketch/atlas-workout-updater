...
async function main() {
  ...
  try {
    ...
    if (['full','dry-run-only','post-switch'].includes(smokeMode)) {
      ...
      const testOk = dd.test_mode === true;
      const noWrite = dd.sheet_written === false || dd.no_write_confirmed === true;
      recordResult(results, 'POST /api/complete-workout test_mode', testOk && noWrite, `test_mode=${testOk} no_write=${noWrite}`);

      if (!testOk || dd.sheet_written === true) throw new Error('Dry-run indicated a write occurred!');
